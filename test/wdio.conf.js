...
exports.config = {
  ...
  //
  specs: [
    `${process.cwd()}/test/features/**/*.feature` // `${process.cwd()}/test/cucumber/features/**/*.feature`
  ],
  // Patterns to exclude.
  exclude: [
  ...
  capabilities: [{
    ...
    chromeOptions: {
      ...
      binary: 'node_modules/puppeteer/.local-chromium/win64-536395/chrome-win32/chrome.exe',
  ...
  cucumberOpts: {
    require: [
      `${process.cwd()}/test/cucumber/stepdefs/search_roles.steps.js`, // './test/cucumber/stepdefs'
  ...
  onPrepare: function onPrepare(/* config, capabilities */) {
    server = importFresh('../src/server/server').serverApi;
    server.start();
  },
  //
  ...
  before: function before(/* capabilities, specs */) {
    const jest = require('jest'); // eslint-disable-line global-require
